...
for tar, tar_dict in dd.items():
    ...
    dd[tar]['pcent_list'] = np.unique(dd[tar]['pcent'])
    ...
    for index, pcent_val in enumerate(dd[tar]['pcent_list']):
        ...
        for iindex, ppcent_val in enumerate(dd[tar]['pcent']):
            # If the central momenta from the two lists then fill root file containers
            if (dd[tar]['pcent_list'][index] == dd[tar]['pcent'][iindex]) :
